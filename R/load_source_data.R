...
#' }
#'
#' @param filename character, csv file or data.frame with the source data.
...
load_source_data <- function(filename,source_factors=NULL,conc_dep,data_type,mix){
  if(grepl(".*.csv", filename)) SOURCE <- read.csv(filename) else SOURCE <- filename # source data (.csv or dataframe)
